...
void solve() {
    ...
    rep(i, 0, n) {
        vector<vector<int>> new_dp(maxK + 1, vector<int>(n + 1, -INF));
        ...
        rep(k, 0, maxK + 1) {
            rep(x, 0, n + 1) {
                ...
                // Option 1: Do nothing
                new_dp[k][x] = max(new_dp[k][x], dp[k][x]);

                // Option 2: Pay 1 coin
                if (k + 1 <= maxK)
                    new_dp[k + 1][x + 1] = max(new_dp[k + 1][x + 1], dp[k][x] + a[i]);

                // Option 3: Pay 2 coins
                if (k + 2 <= maxK)
                    new_dp[k + 2][x + 1] = max(new_dp[k + 2][x + 1], dp[k][x] + a[i] + x);
            }
        }
        dp = new_dp;
    }

    // Now get best sum for each k = 1 to 2N
    rep(k, 1, maxK + 1) {
        int best = -INF;
        rep(x, 0, n + 1) {
            best = max(best, dp[k][x]);
        }
        cout << best << " ";
    }
    cout << endl;
