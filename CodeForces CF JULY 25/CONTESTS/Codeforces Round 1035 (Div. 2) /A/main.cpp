...
void solve() {
    ...
    while (t--) {
        ...
        while (!heap.empty()) {
            ...
            if (current_cost > dist[current_a]) {
                continue;
            }
            ...
            if (next_a <= b + 100) { 
                if (dist.find(next_a) == dist.end() || new_cost < dist[next_a]) {
                    dist[next_a] = new_cost;
                    heap.push({new_cost, next_a});
                }
            }
