...
void solve() {
    ...
    sort(all(nums));
    //after this we just need to find the min abs distance/diffrence can say in this casr
    int lm = INT_MIN; // local maxima
    rep(i,0, n-1){
      if (abs(nums[i+1] - nums[i]) > lm)
      lm = abs(nums[i+1] - nums[i]);
      i++;
    }
