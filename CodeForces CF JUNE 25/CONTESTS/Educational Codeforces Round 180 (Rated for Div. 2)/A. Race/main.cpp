...
void solve() {
    ...
    // The valid b for x must be in (x - (ax - 1), x + (ax - 1))
    int x_low = x - (ax - 1);
    int x_high = x + (ax - 1);
    
    // The valid b for y must be in (y - (ay - 1), y + (ay - 1))
    int y_low = y - (ay - 1);
    int y_high = y + (ay - 1);
    
    // Find the overlapping interval
    int low = max(x_low, y_low);
    int high = min(x_high, y_high);
    
    // Check if there's any integer in [low, high] excluding a
    bool possible = false;
