...
void solve()
{
    ...
    // if n ==  1 or  m == 1 there is only 1 way to get a solution, trivially greedy and maximal

    // if n == m == 2 the greedy leads to the maximal value anyhow so its gonna work anyhow

    // is greedy path always a optimal way to find the solution? NO, fuck no
    // Core conceppts to leaarn here
    //greedy is not always a optimal way to get the max/min solution
    if (n == 1 || m == 1 || (n == 2 && m == 2)) {
        cout << "YES\n"; // Greedy always gives maximum path
    } else {
        cout << "NO\n";  // Greedy may fail
    }
}
