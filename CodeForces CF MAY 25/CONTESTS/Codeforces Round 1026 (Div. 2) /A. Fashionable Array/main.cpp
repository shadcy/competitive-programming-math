...
int main() {
    ...
    while (t--) {
        ...
        int min_ops = n; // worst case: remove all elements
        
        // Try all possible min-max pairs
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                int min_val = vals[i];
                int max_val = vals[j];
                
                // Check if this min-max pair makes a fashionable array
                if ((min_val + max_val) % 2 == 0) {
                    // Count elements that need to be removed
                    int keep = 0;
                    for (int k = 0; k < n; k++) {
                        if (vals[k] >= min_val && vals[k] <= max_val) {
                            keep++;
                        }
                    }
                    int remove = n - keep;
                    min_ops = min(min_ops, remove);
                }
            }
        }
