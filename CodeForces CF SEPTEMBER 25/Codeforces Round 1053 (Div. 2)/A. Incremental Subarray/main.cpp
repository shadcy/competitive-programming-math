...
void solve() {
    ...
    // Check if any element except the first is 1
    bool hasOneAfterFirst = false;
    rep(i, 1, m) {
        if(a[i] == 1) {
            hasOneAfterFirst = true;
            break;
        }
    }
    ...
    int ans;
    if(hasOneAfterFirst) {
        ans = 1; // Special case: only one occurrence
    } else {
        ans = n - a[m-1] + 1; // Default: count suffix positions for last element
    }
